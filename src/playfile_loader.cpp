...
void Playfile_loader::load_data(std::string& fname_, vector<Eigen::Affine3d> &gripper1_affines_, vector<Eigen::Affine3d> &gripper2_affines_, vector<double> &arrival_times_, vector<double> &gripper_angs1_, vector<double> &gripper_angs2_, int& data_size_){
	//ros::NodeHandle nh;
	//fname = fname_;
...
ROS_INFO("MARKER 02"); //TODO DELETE
...
//demo
		...
		cout <<  "record " << i << ":" << arrival_times_[i]<< "\n";

		cout << "x_vecs1[i] is \n" << x_vecs1[i] <<"\n" <<"\n";
	}
//----------
	data_size_ = data.size();
// Collected: gripper1_affines_, gripper2_affines_, arrival_times_, gripper_angs1_, gripper_angs2_ and data_size_ Above 
// Test
	//int i = 0;
...
void Playfile_loader::load_data(std::string& fname_) {
ifstream infile(fname_.c_str());
	if (!infile){ // file couldn't be opened
		cerr << "Error: file could not be opened; halting" << endl;
		exit(1);
...
//----------
	data_size = data.size();
	cout <<  "record " << fname_ << " has been loaded." << "\n";
}
